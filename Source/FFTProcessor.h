...
public:
    ...
    void reset();
    void setScrambleAmount(int n);
    void setLowFreqCutoff(int f, int sampleRate);
...
private:
    ...
    // Step sizes for each bin, generates randomly at start
    int swapBin[numBins];

    // Max swapping step, larger steps means more srambling
    int scrambleAmount = 5;

    // low-cut frequency mapped to bin number;
    int binCutoff = 0;
