...
void FFTProcessor::setScrambleAmount(int n) {
  scrambleAmount = n;
  for (int i = 0; i < numBins; i++) {
    swapBin[i] = rand() % (scrambleAmount+1);
  }
}

void FFTProcessor::setLowFreqCutoff(int f, int sampleRate) {
...
void FFTProcessor::processSpectrum(float* data, int numBins)
{
    ...
    for (int i = binCutoff; i < numBins; ++i) {
        ...
        int swapOffset = swapBin[i];
        if (i - swapOffset > 0) {
            auto tmp = cdata[i];
            cdata[i] = cdata[i - swapOffset];
            cdata[i - swapOffset] = tmp;
        }
